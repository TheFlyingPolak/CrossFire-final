...
void move(struct slot **board, struct player *name)
{
	...
	if( (*name).position->type == HILL)		// Changes player stats depending on the slot they move to. 
	{
		if( (*name).dexterity < 50)
		{
			(*name).strength = (*name).strength - 10;
		}
		else if ( (*name).dexterity >= 60)
		{
			(*name).strength = (*name).strength +10;
		}		
	}
	else if ( (*name).position->type == CITY)
	{
		if( (*name).smartness > 60)
		{
			(*name).magic = (*name).magic + 10;
		}
		else if ( (*name).smartness <= 50)
		{
			(*name).dexterity = (*name).dexterity - 10;
		}		
}
	}
			...
			for ( i=0; i<7; i++)		// Code for finding players that are eligable to be attacked. If so, how many there are and who they are.
			{
				for ( j=0; j<7; j++)
				{
					...
					if ( dist>1 && dist<5)
					{
						...
						{
							if ( players[k].position->row == i && players[k].position->column == j)		// Stores the 'ID' shall we say of the players who are in eligable slots. This ID is simply their number in the array of players. 
							{
								canAttack[count] = k;
								count++;
							}
